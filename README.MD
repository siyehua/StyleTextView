...
##本文主要讲解了TextView控件绘制文字上下留有一定空白原因,以及使用canvas.drawText()绘制文字时,baseLine的该如何确定,如何让文字上下没有留白
...
baseLine:baseLine实际上就是textSize,但TextView上下有一定的空白,导致与size高度不是相等的.
...
```
则文字最顶端的Y坐标topTextY = firstSpace + secondSpace;(如TextView已经设置了android:includeFontPadding="false"则不要加上topSpace)
...
结果(60这个字符串本身距离文字有一点间距,假设使用字母 'l' 刚刚好)
